module.exports = function(userId, done, inputSecret) {
  var self = this;
  ...
  model.fetch($user, function(err) {
    ...
    if (self.options.confirmRegistration || self.options.confirmEmail) {
      var secret = $user.get(self.options.localField + '.' + self.options.secretField)
